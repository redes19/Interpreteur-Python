...
# Environnement global + pile de scopes pour les appels de fonctions
names = {}
scopes = [names]
functions = {}
...
def get_var(name):
    for scope in reversed(scopes):
        if name in scope:
            return scope[name]
    raise Exception("Variable %s not defined" % name)
...
def evalexpr(tree):
    if type(tree) == int:
        ...
        if type(var_name) != str:
            raise Exception("++ s'applique uniquement a des variables")
        ...
        if func_name not in functions:
            raise Exception("Fonction '%s' undefine" % func_name)
        ...
        if len(params) != len(args_list):
            raise Exception(
                "Fonction '%s' attend %d argument(s), %d fourni(s)"
                % (func_name, len(params), len(args_list))
            )

        arg_values = [evalexpr(arg) for arg in args_list]
        local_scope = {}

        for i in range(len(params)):
            local_scope[params[i]] = arg_values[i]
